...
@RoutePage()
class NewEditBrandScreen extends HookConsumerWidget {
  const NewEditBrandScreen({super.key, this.brand});
  final Brand? brand;
  @override
  Widget build(BuildContext context, WidgetRef ref) {
    final en = TextEditingController(text: brand?.en ?? '');
    final ku = TextEditingController(text: brand?.ku ?? '');
    final ar = TextEditingController(text: brand?.ar ?? '');

    final available = useState<bool>(brand?.available ?? true);
    ...
    ref.listen(appSettingsControllerProvider, (previous, next) {
      if (next is NewBrandLoading) {
        context.maybePop();
      } else if (next is NewBrandFailed) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text(
          brand == null
              ? LocaleKeys.screens_newBrand.tr()
              : LocaleKeys.screens_updateBrand.tr(),
        ),
          ...
          child: Column(
            ...
            children: [
              ...
              PrimaryButton(
                ...
                isLoading: controller is NewBrandLoading,
                onPress: () {
                  ...
                  ref
                      ...
                      .newBrand(PostModel(brand: newData), brand);
                },
              ),
            ],
          ),
